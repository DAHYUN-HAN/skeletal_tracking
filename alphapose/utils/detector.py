...
#DetectionLoader(input_source, get_detector(args), cfg, args, batchSize=args.detbatch, mode=mode, queueSize=args.qsize)
#get_detector(args)를 통해 detector 클래스를 가져옴 
class DetectionLoader():
    def __init__(self, input_source, detector, cfg, opt, mode='image', batchSize=1, queueSize=128):
        ...
        if mode == 'image':
            ...
            stream = cv2.VideoCapture(input_source)#동영상 파일에서 프레임을 받아옴
            assert stream.isOpened(), 'Cannot capture source'
            self.path = input_source
            self.datalen = int(stream.get(cv2.CAP_PROP_FRAME_COUNT))#총 프레임 개수 
            self.fourcc = int(stream.get(cv2.CAP_PROP_FOURCC))#코덱 코드 
            self.fps = stream.get(cv2.CAP_PROP_FPS)#프레임 속도 
            self.frameSize = (int(stream.get(cv2.CAP_PROP_FRAME_WIDTH)), int(stream.get(cv2.CAP_PROP_FRAME_HEIGHT)))#프레임 너비, 프레임 높이 
        ...
        self._input_size = cfg.DATA_PRESET.IMAGE_SIZE#256,192
        self._output_size = cfg.DATA_PRESET.HEATMAP_SIZE#64,48
